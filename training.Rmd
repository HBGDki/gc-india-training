...
plot0 <- ggplot( data = deltadplyr, aes (x = obsmean, y = dldt) ) +
  geom_point(alpha=0.1)+
  xlab ("Length (cm)" )+
  ylab ( "Derivative of length (dL/dt)") +
  stat_smooth(method = "gam", formula = y ~ s(x),aes(col="Data Based"),size=1.5)+
  labs(colour="")+
  scale_colour_manual(breaks = c("Data Based"),
    values = c("darkgray"))+
  theme_bw()
plot0
```
We notice that the "velocity" is changing and it is not simply proportional to the current length as we grow taller the velocity becomes slower.
...
We see that the velocity of the exponential model is "linear" and does not describe the data well.
Let us try a model where the rate of growth decelerates with time.
...
fit2 <- nlme(HTCM ~ NLLDmodel(lnLinf, lnL0, lnKgr0, logitKgrdec, lnTTP50, AGEMONTH),
  ...
  control = nlmeControl(maxIter = 20, returnObject = TRUE), verbose = FALSE)
...
plot(fit2)
```
The nonlinear deceleration model has lower AIC better residual patterns. How about the velocity ?
...
plot2 <- plot1 +
  ...
  scale_colour_manual(breaks = c("Data Based", "Exp Model Based", "Nonlinear Deceleration Model"),
...
```
The nonlinear deceleration model better matches the data based velocity.
What about potential covariates that let us explain some of the heterogeneity?
We will extract the random effects of the fit and see if they correlate with some of the covariates we have.
...
summary(fit3)
```
The model takes time to run and it warns about no convergence.
We will still use this model for now. Ideally the model need to be tweaked and kept running longer untill it converges.
As we did for fit2, we will extract the random effects of the fit and see if they correlation has been explained.
...
```
The trend is now more like a flat line and we will now look at both models at the same time. 
...
```
Adding Sex on reduced AIC by ~ 2000 points and fixed the random effect trends.
