...
NOTE: To follow this document online (and to see interactive visualizations live), please visit [this page](https://hbgdki.github.io/gc-india-training/index.html).
...
To begin, we need to load the R packages into our R environment that we will be using to analyze the data. The major packages we will use throughout are the [hbgd](http://hbgdki.github.io/hbgd/) package and packages in the "Tidyverse". This collection of packages provides many powerful methods for many general-purpose data manipulation and visualization tasks. A free, excellent in-depth resource on packages in the Tidyverse can be found [here](http://r4ds.had.co.nz/).
...
We will use the "ggplot2" package, whish is one of the most popular add-on packages for R, providing a powerful, flexible and well-thought-out platform to create data visualizations you can customize to your heart's content. A good reference for learning ggplot2 can be found [here](http://r4ds.had.co.nz/data-visualisation.html).
...
We can choose which trajectories to visualize in a more meaningful way by adding some metrics, which we call "cognostics" (TODO: define) to the data. Using the hbgd package, this can be done with a function `add_longi_cogs()`.
...
Now we can see clearly the phenomenon of these subjects having a significant recover after dropping in length halfway into their first year.
